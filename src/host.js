// dsh-ghCLI · Host 半区（动态 Cordis Plugin 的 code.host 函数体）
// 模块映射（readme.txt 第 4 节）：
...
//   routes          -> 底部 harness.handle(...) 注册的 gh.* RPC 接口
// 依赖：ctx.subprocess（执行）、ctx.timer（超时/取消）、ctx.fs（目录探测），均 optional。
return {
  apply(ctx) {
    ...
    harness.handle('gh.status', async () => {
      try {
    ...
    harness.handle('gh.install', async (args) => {
      try {
        return await installGh((args && args.opId) || undefined)
      } catch (e) {
        return { ok: false, error: String((e && e.message) || e) }
      }
    })

    harness.handle('gh.setPath', async (args) => {
      try {
        const a = args || {}
        return await setManualPath(String(a.path || ''))
      } catch (e) {
        return { ok: false, error: String((e && e.message) || e) }
      }
    })

    harness.handle('gh.addPath', async (args) => {
      try {
        const a = args || {}
    ...
    harness.handle('gh.auth.login', async (args) => {
      try {
    ...
    harness.handle('gh.auth.switch', async (args) => {
      try {
    ...
    harness.handle('gh.auth.logout', async (args) => {
      try {
    ...
    harness.handle('gh.auth.refresh', async (args) => {
      try {
    ...
    harness.handle('gh.git.save', async (args) => {
      try {
        const a = args || {}
    ...
    harness.handle('gh.repo.current', async (args) => {
      try {
        const a = args || {}
        return await repoCurrent(String(a.workdir || '').trim() || undefined)
      } catch (e) {
        return { ok: false, error: String((e && e.message) || e) }
      }
    })

    harness.handle('gh.repo.clone', async (args) => {
      try {
    ...
    harness.handle('gh.repo.status', async (args) => {
      try {
        const a = args || {}
        return await repoStatus(String(a.repo || '').trim())
      } catch (e) {
        return { ok: false, error: String((e && e.message) || e) }
      }
    })

    harness.handle('gh.repo.fork', async (args) => {
      try {
    ...
    harness.handle('gh.repo.create', async (args) => {
      try {
    ...
    harness.handle('gh.repo.open', async (args) => {
      try {
        const a = args || {}
        return await repoOpenRemote(String(a.workdir || '').trim() || undefined)
      } catch (e) {
        return { ok: false, error: String((e && e.message) || e) }
      }
    })

    harness.handle('gh.repo.push', async (args) => {
      try {
        const a = args || {}
        return await repoPush(String(a.workdir || '').trim() || undefined)
      } catch (e) {
        return { ok: false, error: String((e && e.message) || e) }
      }
    })

    harness.handle('gh.repo.pull', async (args) => {
      try {
        const a = args || {}
        return await repoPull(String(a.workdir || '').trim() || undefined)
      } catch (e) {
        return { ok: false, error: String((e && e.message) || e) }
      }
    })

    harness.handle('gh.repo.pr', async (args) => {
      try {
        const a = args || {}
        return await repoPrCreate(String(a.workdir || '').trim() || undefined)
      } catch (e) {
        return { ok: false, error: String((e && e.message) || e) }
      }
    })

    harness.handle('gh.op.cancel', async (args) => {
      try {
        const a = args || {}
        const op = state.activeOps.get(String(a.opId || ''))
    ...
    // P0：apply 钩子触发 gh --version 预热检测（readme 1.1）
