// dsh-ghCLI · Client 半区（标准插件 exports["./client"] 束）
// 原动态 Cordis Plugin（ghcli-1）代码已迁移为标准插件包形态：
//   本文件整体是 window.__ModuleLoader__.load 的 module-loader 束（id 与包名一致），
//   gh.* RPC 由动态的 host.call() 改为同源 fetch 到 /api/dsh-ghcli/* 路由。
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /** 注入本插件的样式表（module-loader 的 data-plugin-css 约定，重复执行幂等）。 */
    function insertStyles(css) {
      if (typeof document === 'undefined') return
      let tag = document.querySelector('style[data-plugin-css="dsh-ghcli"]')
      if (!tag) {
        tag = document.createElement('style')
        tag.setAttribute('data-plugin-css', 'dsh-ghcli')
        document.head.appendChild(tag)
      }
      tag.textContent = css
    }

    /** Client -> Host RPC：POST 到 /api/dsh-ghcli/<method>，响应即 handler 返回值（与旧 host.call 同契约）。 */
    function rpc(method, args) {
      return fetch('/api/dsh-ghcli/' + method, {
        method: 'POST',
...
`)
      ...
      function GhSettings(props) {
        ...
        async function call(method, args, extra = {}) {
          ...
          try {
            ...
            const res = await rpc(method, payload)
            if (res && res.ok === false) {
        ...
        return h('div', { className: 'ghcli-root' },
          ...
          busyLabel ? h('div', { className: 'ghcli-row' },
            ...
            opId ? h('button', { className: 'ghcli-btn danger', onClick: () => { rpc('gh.op.cancel', { opId }).catch(() => {}) } }, '取消') : null,
          ) : null,
      ...
      function StatusTab(p) {
        ...
        React.useEffect(() => {
          let alive = true
          rpc('gh.net.status', {}).then((res) => { if (alive && res && res.ok) setNet(res) }).catch(() => {})
          return () => { alive = false }
        }, [])
        async function refreshNet() {
          const r = await rpc('gh.net.status', {})
          if (r && r.ok) setNet(r)
        }
      ...
      function RepoTab(p) {
        ...
        React.useEffect(() => {
          if (!p.workdir) return
          let alive = true
          rpc('gh.repo.current', { workdir: p.workdir }).then((res) => { if (alive && res) setRepoInfo(res) }).catch(() => {})
          return () => { alive = false }
        }, [p.workdir])

        async function refreshRepo() {
          if (!p.workdir) return
          const r = await rpc('gh.repo.current', { workdir: p.workdir })
          if (r) setRepoInfo(r)
        }
